...
There are two different types of transactions in the Blackfury Ethereum EVM: value transfers and contract executions. A value transfer simply moves
...
tracing solves. Blackfury Ethereum's interface traces transactions by re-running them locally and collecting data about precisely what was executed by the EVM.

## State Availability

In its simplest form, tracing a transaction entails requesting the Blackfury Ethereum node to reexecute the desired transaction with 
varying degrees of data collection and have it return an aggregated summary. In order for a Blackfury Ethereum node to reexecute a transaction, 
all historical state accessed by the transaction must be available. 

By Default, all Blackfury Ethereum nodes are archival nodes.
It is possible to run a Blackfury Ethereum light client but for EVM tracing, a full node is required.
...
Blackfury Ethereum has the ability to produce basic transaction traces in the form of raw EVM opcode traces. These traces contain detailed information about every EVM instruction executed during a transaction, 
...
The tracing API of Blackfury Ethereum allows the user to specify a tracer parameter, 
...
resource-intensive to process. To address these issues, Blackfury Ethereum includes 
...
Besides the built-in tracers, Blackfury Ethereum also allows users to create custom tracers 
