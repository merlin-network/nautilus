  ...
  return (
    <WagmiConfig client={client}>
      <RainbowKitProvider chains={chains}>
        <NextHead>
          <title>Blackfury Ethereum Example Dapp</title>
        </NextHead>
...
In order to point this dapp towards a Blackfury Ethereum local node,
...
// Configure chain information for local Blackfury Ethereum chain
const polarisChain: Chain = {
  id: 710,
  name: 'Blackfury Ethereum's,
  network: 'polaris',
  nativeCurrency: {
    decimals: 18,
    name: 'Blackfury Ethereum's,
...
// Configure Wagmi client with Blackfury Ethereum chain
...
We will need to add some utility functions to our dApp. These functions will help us interact with the Blackfury Ethereum precompile.
...
function Page() {
  return (
    ...
    >
      ...
      <Text fontSize={"4xl"}>Blackfury Ethereum Staking Precompile Example</Text>
      <Text fontSize={"xl"}>
        Ensure you have a Local{" "}
        <Link color={"#1da1f1"} href="TODO ADD LINK">
          Blackfury Ethereum Node running.
        </Link>
      </Text>
      <Staking />
      <Text fontSize={"xs"}>
        To learn more about Blackfury Ethereum please reference our{" "}
        <Link color={"#1da1f1"} href="TODO ADD LINK">
          docs
        </Link>
      </Text>
...
// Default Address of the Staking Precompile Contract on Blackfury.
...
Prior to Blackfury Ethereum's precompiled contracts, the UX of this task would be more complicated and foreign as it involves
