...
on a local Blackfury Ethereum EVM network.
...
- Basic understanding of the [Blackfury Ethereum's architecture](../../architecture)
...
- You have [installed
  ...
  walk-through.
- You have a running instance of Blackfury Ethereum EVM. See [Local Machine](../../running-a-node/running-locally) to run Blackfury Ethereum locally.

## Blackfury Ethereum Liquid Staking Smart Contract Example
...
### Add Configurations For Local Blackfury Ethereum

<Callout emoji="💡">
  Ensure that you have a local instance of Blackfury Ethereum Running. See [Local
  Machine](../../running-a-node/running-locally) to run Blackfury Ethereum locally.
</Callout>
...
To test the configurations above, let's deploy the generated contract in the "Counter.sol" file to the Blackfury Ethereum localnet:

```bash
FOUNDRY_PROFILE=blackfury_local forge create Counter --private-key=xxxxxxxxxxx
```
...
```bash
FOUNDRY_PROFILE=blackfury_local forge create src/LiquidStaking.sol:LiquidStaking --private-key=xxxxxxxxxxx --constructor-args  “<name>“ “<symbol>” “<staking_precompiled_contract_address>” “<validator_address>”
```
...
```bash
FOUNDRY_PROFILE=blackfury_local forge script src/examples/Deploy.s.sol:Deploy --broadcast  --private-key=xxxxxxx --rpc-url=xxxx --gas-limit 10000000
```
...
```bash
FOUNDRY_PROFILE=blackfury_local cast send --private-key=xxxxxxxxxxx <deployed_contract_address> ‘delegate()’ 1
```

###  🎉 Congratulations! 🎉

Congratulations! You have successfully created and deployed a Liquid Staking contract on Blackfury Ethereum via Foundry!
